...
public class ModelRepository
{
    ...
    public async Task<Model3D?> GetByIdAsync(Guid modelId, CancellationToken ct = default)
        => await _db.Models
            .Include(m => m.Components)
            .FirstOrDefaultAsync(m => m.ModelId == modelId, ct);

    public async Task<Model3D> CreateAsync(Model3D model, CancellationToken ct = default)
    {
    ...
    public async Task<List<Model3D>> SearchByNameAsync(string query, int limit = 10, CancellationToken ct = default)
        => await _db.Models
